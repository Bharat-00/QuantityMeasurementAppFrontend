...
function passwordMatchValidator(control: AbstractControl) {
  ...
  if (password && confirm && password !== confirm) {
    ...
    if (errors) {
      ...
      if (!Object.keys(errors).length) control.get('confirmPassword')?.setErrors(null);
    }
  }
  return null;
}
...
export class SignupComponent {
  ...
  onSubmit(): void {
    ...
    this.authService.signup({ username, email, password }).subscribe({
      next: () => {
        this.snackBar.open('Account created! Please sign in.', '✓', { duration: 3000, panelClass: 'snack-success' });
